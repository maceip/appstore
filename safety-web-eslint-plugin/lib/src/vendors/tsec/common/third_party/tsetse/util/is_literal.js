...
function isLiteral(typeChecker, node) {
    if (ts.isBinaryExpression(node) &&
        ...
        return (isLiteral(typeChecker, node.left) && isLiteral(typeChecker, node.right));
    }
    ...
    const hasCasts = (0, ast_tools_1.findInChildren)(node, (n) => ts.isAsExpression(n) && !ts.isConstTypeReference(n.type));
